...
authRouter.post("/register", async (req: Request, res: Response) => {
  ...
  const companiesResult = await db.query(
    ...
    ["Default companies", "Default address"]
  );
  ...
  await db.query(
    "insert into menus_locations (menus_id, locations_id, is_available) select * from unnest ($1::int[], $2::int[], $3::boolean[])",
    [
  ...
  const defaultAddonCategoriesResult = await db.query(
    "insert into addon_categories (name, is_required) values ('Drinks', false), ('Sizes', true) returning *"
  );
